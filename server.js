...
const upload = multer({ storage: multer.memoryStorage() }); // メモリ上で処理（Renderに最適）
...
app.get("/auth/discord/callback", async (req, res) => {
  ...
  try {
    ...
    if (memberRes.status === 404) {
      ...
      return res.status(403).send("Error: サーバーに参加していません。");
    }
...
app.post("/api/recruit/apply", async (req, res) => {
  ...
  try {
    ...
    req.session.applicant = null; // 送信後はセッションリセット
...
app.post("/api/software", auth, async (req, res) => { try { const b = req.body; await supabase.from('software').insert([{ name:b.name, version:b.version, description:b.description, zip_path:b.zip_url||"", apk_path:b.apk_url||"", is_beta:b.is_beta?1:0, is_update:b.is_update?1:0, is_maintenance:b.is_maintenance?1:0, is_original:b.is_original?1:0, is_thirdparty:b.is_thirdparty?1:0, tags:JSON.stringify(b.tags||[]) }]); res.json({ success: true }); } catch(e) { res.status(500).json({error: e.message}); } });
app.put("/api/software/:id", auth, async (req, res) => { try { const b = req.body; await supabase.from('software').update({ name:b.name, version:b.version, description:b.description, zip_path:b.zip_url||"", apk_path:b.apk_url||"", zip_downloads:parseInt(b.zip_downloads)||0, apk_downloads:parseInt(b.apk_downloads)||0, is_beta:b.is_beta?1:0, is_update:b.is_update?1:0, is_maintenance:b.is_maintenance?1:0, is_original:b.is_original?1:0, is_thirdparty:b.is_thirdparty?1:0, tags:JSON.stringify(b.tags||[]) }).eq('id', req.params.id); res.json({ success: true }); } catch(e) { res.status(500).json({error: e.message}); } });
...
app.post("/api/download/:id/:type", async (req, res) => { try { const { id, type } = req.params; const { data: item } = await supabase.from('software').select('*').eq('id', id).single(); if(!item) return res.status(404).json({error:"Not found"}); if(item.is_maintenance===1) return res.status(403).json({error:"Maintenance"}); if (type === 'zip' && item.zip_path) { await supabase.from('software').update({ zip_downloads: item.zip_downloads + 1 }).eq('id', id); return res.json({ link: item.zip_path }); } else if (type === 'apk' && item.apk_path) { await supabase.from('software').update({ apk_downloads: item.apk_downloads + 1 }).eq('id', id); return res.json({ link: item.apk_path }); } else return res.status(404).json({error:"File not found"}); } catch(e) { res.status(500).json({error: "Server error"}); } });
...
app.post("/api/sns", async (req, res) => { try { await supabase.from('sns_promotions').insert([{ user_name:req.body.user_name||"Anonymous", sns_type:req.body.sns_type||"other", url:req.body.url, description:req.body.description }]); res.json({ success: true }); } catch(e) { res.status(500).json({error: e.message}); } });
